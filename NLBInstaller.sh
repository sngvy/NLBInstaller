...
# Не даём запускаться параллельно с предыдущей ещё не завершившейся копией
# (актуально, если classify_ip с сетевыми запросами не укладывается в интервал cron)
exec 200>/var/lock/update-nlb.lock
flock -n 200 || exit 0
...
# Учёт уже переоценённых (через proxycheck/ip-api) забаненных IP — чтобы не
# дёргать сторонние сервисы повторно на каждый прогон cron для одного и того
# же давно забаненного адреса. Запись появляется только когда IP реально
# прошёл classify_ip и решение было "блокировать"; при снятии бана (whitelist
# или повторная классификация как мобильного) запись удаляется.
REASSESSED_FILE="/usr/local/etc/nlb_reassessed.list"
DECISIONS_LOG="/var/log/nlb_decisions.log"
WHITELIST_SOURCE_URL="https://raw.githubusercontent.com/sngvy/NLBInstaller/main/nlb_whitelist.txt"

touch "$REASSESSED_FILE" 2>/dev/null
...
# ------------------------------------------------------------------
# ip_in_whitelist <ip> — 0, если IP входит в одну из подсетей WHITELIST_FILE.
# Не обращается ни к каким внешним сервисам — чистая локальная проверка.
# ------------------------------------------------------------------
ip_in_whitelist() {
    local check_ip="$1"
    [ -f "$WHITELIST_FILE" ] && [ -s "$WHITELIST_FILE" ] || return 1
    python3 -c "
...
try:
    ...
    pass
sys.exit(1)
" "$check_ip" "$WHITELIST_FILE" 2>/dev/null
}

# ------------------------------------------------------------------
# Учёт уже переоценённых IP (REASSESSED_FILE) — см. комментарий у объявления.
# ------------------------------------------------------------------
is_reassessed() {
    [ -f "$REASSESSED_FILE" ] && grep -qxF "$1" "$REASSESSED_FILE" 2>/dev/null
}
mark_reassessed() {
    echo "$1" >> "$REASSESSED_FILE"
}
unmark_reassessed() {
    [ -f "$REASSESSED_FILE" ] || return 0
    grep -vxF "$1" "$REASSESSED_FILE" > "${REASSESSED_FILE}.tmp" 2>/dev/null \
        && mv "${REASSESSED_FILE}.tmp" "$REASSESSED_FILE"
}
...
if [ "$MODE" = "ufw" ]; then
    ...
    BANNED_IPS=$(ufw status | grep 'NLB-Block' | grep -oE '([0-9]{1,3}\.){3}[0-9]{1,3}|([0-9a-fA-F]{0,4}:){2,7}[0-9a-fA-F]{0,4}' | awk '!x[$0]++')

    # Подчищаем учёт переоценённых от IP, которых уже нет в бане (сняты вручную,
    # правилом истёк срок и т.п.) — чтобы файл не рос бесконечно мусором.
    if [ -s "$REASSESSED_FILE" ]; then
        PRUNED=$(mktemp)
        while read -r r_ip; do
            [ -z "$r_ip" ] && continue
            echo "$BANNED_IPS" | grep -qxF "$r_ip" && echo "$r_ip" >> "$PRUNED"
        done < "$REASSESSED_FILE"
        mv "$PRUNED" "$REASSESSED_FILE"
    fi

    # --- Whitelist + переоценка уже забаненных IP, единым проходом ---
    # Порядок важен: сперва локальная проверка по whitelist (без единого
    # обращения к proxycheck/ip-api), и только если IP не в whitelist и ещё
    # ни разу не переоценивался — идём во внешние сервисы. Уже переоценённые
    # ранее IP пропускаем молча, пока не попадут в whitelist.
    echo "$BANNED_IPS" | while read -r b_ip; do
        [ -z "$b_ip" ] && continue

        if ip_in_whitelist "$b_ip"; then
            ufw delete deny from "$b_ip" 2>/dev/null
            unmark_reassessed "$b_ip"
            continue
        fi

        if is_reassessed "$b_ip"; then
            continue
        fi

        if ! classify_ip "$b_ip"; then
            ufw delete deny from "$b_ip" 2>/dev/null
        else
            mark_reassessed "$b_ip"
        fi
    done
    # -----------------------------------------
    ...
    # Собираем текущий полный список забаненных (обе таблицы) для подчистки учёта переоценки.
    BANNED_IPS_ALL=""
    for CMD in iptables ip6tables; do
        BANNED_IPS_ALL="$BANNED_IPS_ALL"$'\n'"$($CMD -t raw -S PREROUTING 2>/dev/null | grep -w DROP | grep -oE '([0-9]{1,3}\.){3}[0-9]{1,3}|([0-9a-fA-F]{0,4}:){2,7}[0-9a-fA-F]{0,4}')"
    done
    BANNED_IPS_ALL=$(echo "$BANNED_IPS_ALL" | sed '/^$/d' | awk '!x[$0]++')
    ...
    if [ -s "$REASSESSED_FILE" ]; then
        ...
        while read -r r_ip; do
            [ -z "$r_ip" ] && continue
            echo "$BANNED_IPS_ALL" | grep -qxF "$r_ip" && echo "$r_ip" >> "$PRUNED"
        done < "$REASSESSED_FILE"
        mv "$PRUNED" "$REASSESSED_FILE"
    fi
    ...
    # обращения к proxycheck/ip-api), и только если IP не в whitelist и ещё
    # ни разу не переоценивался — идём во внешние сервисы. Уже переоценённые
    # ранее IP пропускаем молча, пока не попадут в whitelist. Идём по каждой
    # таблице отдельно — для -D нужна верная команда (iptables/ip6tables).
    for CMD in iptables ip6tables; do
        $CMD -t raw -S PREROUTING 2>/dev/null | grep -w DROP | grep -oE '([0-9]{1,3}\.){3}[0-9]{1,3}|([0-9a-fA-F]{0,4}:){2,7}[0-9a-fA-F]{0,4}' | awk '!x[$0]++' | while read -r b_ip; do
            if ip_in_whitelist "$b_ip"; then
